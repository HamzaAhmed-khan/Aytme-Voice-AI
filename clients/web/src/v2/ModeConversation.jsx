...
import { 
  ...
  Settings2, Globe2, UserPlus, Shield
} from 'lucide-react';
import { LANGUAGES } from './languages';

export default function ModeConversation() {
  ...
  const handleStart = async (e, autoJoin) => {
    ...
    try {
      const room = await roomService.createRoom({
        ...
        policy: { intent_mode: 'conversation' }
      });
      
      if (autoJoin) {
        // Navigate to the stage with conversation intent
        navigate(`/v2/room/${room.id}?mode=conversation&host=${formData.hostName}&role=speaker`);
      } else {
        toast.success('Session created! You can join it later from the Rooms tab.');
        navigate('/v2/rooms');
      }
    } catch (err) {
      ...
      if (code === 'QUOTA_EXCEEDED') {
        ...
        toast.error(msg ? `Error: ${msg}` : 'Failed to initialize session. Please try again.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="v2-page max-w-4xl mx-auto space-y-12 py-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="text-center space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-v2-accent/10 border border-v2-accent/20 text-v2-accent text-[11px] font-bold uppercase tracking-widest">
          <Sparkles size={14} />
          High-Fidelity AI Mode
        </div>
        <h1 className="text-5xl font-bold tracking-tighter text-v2-text uppercase">Conversation Mode</h1>
        <p className="text-v2-muted max-w-xl mx-auto font-medium">
          Real-time 1-on-1 translation with ultra-low latency. 
          Perfect for interviews, meetings, and private consultations.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-10">
        <form onSubmit={handleStart} className="md:col-span-7 v2-card space-y-8 p-10 shadow-2xl">
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="v2-label flex items-center gap-2">
                <UserPlus size={14} /> Host Identifier
              </label>
              <input 
                type="text" 
                className="v2-input h-14"
                placeholder="Your display name in the session"
        ...
        <div className="md:col-span-5 space-y-6">
          <div className="v2-card bg-emerald-50/50 border-emerald-100 p-6">
            <h3 className="text-emerald-700 font-bold uppercase text-xs tracking-widest mb-4 flex items-center gap-2">
              <Shield size={14} /> Security Features
            </h3>
            <ul className="space-y-4">
              <li className="flex gap-3 text-sm text-emerald-900/70 font-medium">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 flex-shrink-0" />
                End-to-end encrypted audio via LiveKit
              </li>
              <li className="flex gap-3 text-sm text-emerald-900/70 font-medium">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 flex-shrink-0" />
                Private WebRTC session (Encrypted)
              </li>
              <li className="flex gap-3 text-sm text-emerald-900/70 font-medium">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 mt-1.5 flex-shrink-0" />
                Isolated translation worker
              </li>
            </ul>
          </div>

          <div className="v2-card bg-indigo-50/50 border-indigo-100 p-6">
            <h3 className="text-indigo-700 font-bold uppercase text-xs tracking-widest mb-4 flex items-center gap-2">
              <MessageSquare size={14} /> Integration
            </h3>
            <p className="text-sm text-indigo-900/70 leading-relaxed font-medium">
              This mode activates the <strong>Integrated AI Pipeline</strong>, providing both live captions and translated voice output for a natural conversational flow.
            </p>
          </div>
        </div>
      </div>
